...
function get_the_post_custom_thumbnail(int $post_id, string $size = 'featured-thumbnail', array $additional_attributes = []): string
{
    ...
    if (has_post_thumbnail($post_id)) {
        ...
        $attributes = array_merge($additional_attributes, $default_attribues);
...
function aquila_excerpt_more($more = '')
{
    ...
    if (!is_single()) {
        $more = sprintf(
            '<button class="mt-4 btn btn-info"><a class="aquila-read-more text-white text-decoration-none" href="%1$s">%2$s</a></button>',
